...
hybrid) all funnel through ``PaymentService.compute_payable``. The table /
KPIs / contributor detail / CSV export all read from it, so anything that
goes wrong here propagates everywhere. The tests below pin one assertion
per model and the cross-cutting helpers (``effective_comp_model``,
``prorated_salary``, ``_comp_filter_from_body``, ``passes_comp_filter``).
...
def test_prorated_salary_no_salary_returns_zero():
    ...
    assert PaymentService.prorated_salary(u, date(2026, 4, 1), date(2026, 4, 30)) == 0.0
...
def test_prorated_salary_full_month_returns_full_salary():
    ...
    assert PaymentService.prorated_salary(u, date(2026, 4, 1), date(2026, 4, 30)) == 3000.00


def test_prorated_salary_half_month_returns_half_salary():
    """First half of April (1st–15th, 15 days of 30) at $3000 → $1500."""
    u = _FakeUser(monthly_salary=Decimal("3000"))
    assert PaymentService.prorated_salary(u, date(2026, 4, 1), date(2026, 4, 15)) == 1500.00
...
def test_prorated_salary_february_short_month_still_full_when_inclusive():
    ...
    assert PaymentService.prorated_salary(u, date(2026, 2, 1), date(2026, 2, 28)) == 2800.00
...
def test_prorated_salary_december_anchor_handles_year_rollover():
    ...
    assert PaymentService.prorated_salary(u, date(2026, 12, 1), date(2026, 12, 31)) == 3100.00
...
def test_passes_comp_filter_explicit_multi_model_filter():
    ...
    assert PaymentService.passes_comp_filter("salaried", f) is True
    assert PaymentService.passes_comp_filter("per_task", f) is False
    assert PaymentService.passes_comp_filter("hybrid", f) is False
