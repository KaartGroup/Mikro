...
def make_activity_row(activity=None, subcategory_name=None, **kwargs):
    ...
    return SimpleNamespace(
        activity=activity, subcategory_name=subcategory_name, **kwargs
    )
...
def test_validating_named_non_community_subcategory_maps_to_qc():
...
def test_documentation_wiki_documentation_maps_to_community_documentation():
    row = make_activity_row(
        activity="documentation", subcategory_name="Wiki Documentation"
    )
    assert (
        _categorize_activity(row.activity, row.subcategory_name)
...
def test_other_community_outreach_maps_to_community_outreach():
    ...
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "community_outreach"
    )


def test_other_does_fuzzy_matching_for_community_outreach():
    row = make_activity_row(
        activity="other", subcategory_name="Community outreach - IWD Event Indonesia"
    )
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "community_outreach"
    )
...
def test_meeting_community_maps_to_community_meeting():
    ...
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "community_meeting"
    )


def test_meeting_internal_maps_to_meeting():
    row = make_activity_row(
        activity="meeting", subcategory_name="Internal Team Members"
    )
    assert _categorize_activity(row.activity, row.subcategory_name) == "meeting"
...
def test_training_community_maps_to_community_training():
    ...
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "community_training"
    )
...
def test_imagery_capture_maps_to_imagery_capture():
    row = make_activity_row(
        activity="imagery_capture", subcategory_name="Narrow Road Imagery Collection"
    )
...
def test_project_creation_maps_to_project_creation():
    ...
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "project_creation"
    )


def test_project_creation_community_project_maps_to_project_creation():
    row = make_activity_row(
        activity="project_creation", subcategory_name="Community Project"
    )
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "project_creation"
    )
...
def test_documentation_non_wiki_maps_to_documentation():
    row = make_activity_row(
        activity="documentation", subcategory_name="Project Workflow Documentation"
    )
    assert _categorize_activity(row.activity, row.subcategory_name) == "documentation"
...
def test_other_community_qc_notes_maps_to_community_outreach():
    row = make_activity_row(
        activity="other", subcategory_name="Community QC/Notes Review"
    )
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "community_outreach"
    )
...
def test_other_community_project_updates_maps_to_community_outreach():
    row = make_activity_row(
        activity="other", subcategory_name="community project/challenge updates"
    )
    assert (
        _categorize_activity(row.activity, row.subcategory_name) == "community_outreach"
    )
...
def activity_rows(*pairs):
    ...
    for p in pairs:
        ...
        rows.append(
            make_activity_row(activity=activity, subcategory_name=subcategory_name)
        )
    return rows
