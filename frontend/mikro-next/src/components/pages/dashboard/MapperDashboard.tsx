      ...
      <CardContent className="p-0">
        <div className="overflow-x-auto">
          <table className="w-full text-sm" style={{ minWidth: 600 }}>
            ...
            <tbody className="divide-y divide-border">
              {projects.slice((page - 1) * ROWS_PER_PAGE, page * ROWS_PER_PAGE).map((project) => (
                <tr
                  key={project.id}
                  className="cursor-pointer hover:bg-muted/50 transition-colors"
                  onDoubleClick={() =>
                    window.open(getProjectExternalUrl(project.id, project.source), "_blank", "noopener,noreferrer")
                  }
                >
                  <td className="px-2 py-2 font-medium">
                    <Link href={`/projects/${project.id}`} className="text-kaart-orange hover:underline" title="View project details">
                      {project.name}
                    </Link>
                  </td>
        ...
        {projects.length > ROWS_PER_PAGE && (
          <div className="flex items-center justify-between mt-4 px-4 pb-4 text-sm text-muted-foreground">
            <span>Showing {(page - 1) * ROWS_PER_PAGE + 1}–{Math.min(page * ROWS_PER_PAGE, projects.length)} of {projects.length}</span>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" disabled={page === 1} onClick={() => setPage((p) => p - 1)}>Previous</Button>
              <span className="flex items-center px-2">Page {page} of {Math.ceil(projects.length / ROWS_PER_PAGE)}</span>
              <Button variant="outline" size="sm" disabled={page === Math.ceil(projects.length / ROWS_PER_PAGE)} onClick={() => setPage((p) => p + 1)}>Next</Button>
            </div>
          </div>
        )}
      </CardContent>
...
export function MapperDashboard({ isValidator = false }: Props) {
  ...
  const { data: userStats, loading: userStatsLoading, refetch: refetchUserStats } =
    useUserDashboardStats(!isValidator);
  const { data: validatorStats, loading: validatorStatsLoading, refetch: refetchValidatorStats } =
  ...
  const { data: userProjectsData, loading: userProjectsLoading, refetch: refetchUserProjects } =
    useUserProjects(!isValidator);
  const { data: validatorProjectsData, loading: validatorProjectsLoading, refetch: refetchValidatorProjects } =
    useValidatorProjects(isValidator);

  const projectsLoading = isValidator ? validatorProjectsLoading : userProjectsLoading;
  const refetchProjects = isValidator ? refetchValidatorProjects : refetchUserProjects;
  ...
  const displayProjects = isValidator ? projects : (userProjectsData?.org_active_projects || []);
  ...
  const tasksValidated = isValidator ? vs?.tasks_validated : userStats?.validated_tasks;
  const tasksInvalidated = isValidator ? vs?.tasks_invalidated : userStats?.invalidated_tasks;
  const requestsTotal = isValidator ? vs?.requests_total : userStats?.requests_total;
  const payoutsTotal = isValidator ? (vs?.paid_total ?? vs?.payouts_total) : userStats?.payouts_total;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ gridColumn: "span 1" }}>
          <TimeTrackingWidget
            projects={projects.map((p) => ({
              ...
              last_worked_on: (p as Project & { last_worked_on?: string | null }).last_worked_on ?? null,
            }))}
          />
        </div>
      ...
      {isValidator && vs?.self_validated_count != null && vs.self_validated_count > 0 && (
        <div className="rounded-lg bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="font-medium text-yellow-800 dark:text-yellow-200">Self-Validation Warning</p>
          </div>
          <p className="text-sm text-yellow-700 dark:text-yellow-300">
            {formatNumber(vs.self_validated_count).text} task(s) you validated were mapped by you and are not eligible for payment.
          </p>
        </div>
      )}
      ...
      {/* Main Stats Cards */}
      <div
        style={{ display: "grid", gap: 16, gridTemplateColumns: paymentsVisible ? "repeat(4, 1fr)" : "repeat(3, 1fr)" }}
      ...
      >
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  {isValidator
                    ...
                    : <><Val>{formatNumber(userStats?.total_contributions_for_month)}</Val> this month</>}
                </p>
              </>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{isValidator ? "Tasks Approved" : "Tasks Validated"}</CardTitle>
            <StatCardLink href="/projects" label="View your projects">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                {(requestsTotal ?? 0) > 0 ? "Available Balance" : "Payable Total"}
              </CardTitle>
              ...
              {payableLoading || statsLoading ? (
                ...
                <>
                  <div className="text-2xl font-bold">
                    <Val>{formatCurrency(payable?.payable_total ?? vs?.payable_total)}</Val>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {(requestsTotal ?? 0) > 0 ? "Request pending" : "Available for payout"}
                  </p>
                </>
              )}
            </CardContent>
      ...
      {isValidator && (
        <div>
          ...
          <div
            style={{ display: "grid", gap: 16, gridTemplateColumns: paymentsVisible ? "repeat(3, 1fr)" : "repeat(2, 1fr)" }}
            className="grid-stats"
          >
              ...
              <CardContent>
                {statsLoading ? <Skeleton className="h-8 w-16" /> : (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground">Tasks approved by you</p>
                  </>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent>
                {statsLoading ? <Skeleton className="h-8 w-16" /> : (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground">Sent back for fixes</p>
                  </>
                )}
              </CardContent>
            </Card>
              ...
              <Card className="border-green-200 bg-green-50/50 dark:bg-green-950/20">
                ...
                <CardContent>
                  {statsLoading ? <Skeleton className="h-8 w-20" /> : (
                    <>
                      <div className="text-2xl font-bold text-green-600">
                        <Val>{formatCurrency(vs?.calculated_validation_earnings ?? vs?.validation_payable_total)}</Val>
                      </div>
                      <p className="text-xs text-muted-foreground">From validation work</p>
                    </>
                  )}
                </CardContent>
              </Card>
      ...
      {paymentsVisible && (
        <div
          style={{ display: "grid", gap: 12, gridTemplateColumns: isValidator ? "repeat(4, 1fr)" : "repeat(5, 1fr)" }}
          className="grid-earnings"
        >
          {[
            { label: "Mapping Earnings", value: payable?.mapping_earnings ?? vs?.mapping_payable_total ?? userStats?.mapping_payable_total, color: "#ff6b35", link: "/payments", linkLabel: "View payments" },
            { label: "Validation Earnings", value: payable?.validation_earnings ?? vs?.validation_payable_total ?? userStats?.validation_payable_total, color: "#2563eb", link: "/payments", linkLabel: "View payments" },
            ...
            <Card key={label} style={{ padding: 0 }}>
              <div style={{ padding: "10px 14px", position: "relative" }}>
                ...
                {payableLoading ? (
                  ...
                  <div style={{ fontSize: isValidator ? 20 : 18, fontWeight: 700, color }}>
          ...
          {!isValidator && (
            <Card style={{ padding: 0 }}>
              <div style={{ padding: "10px 14px", position: "relative" }}>
                <div style={{ position: "absolute", top: 8, right: 10 }}>
                  <StatCardLink href="/checklists" label="View your checklists" />
                </div>
                <p style={{ fontSize: 11, color: "#6b7280", marginBottom: 2 }}>Checklist Earnings</p>
                {payableLoading ? <Skeleton className="h-6 w-20" /> : (
          ...
          <Card style={{ padding: 0 }}>
            <div style={{ padding: "10px 14px", position: "relative" }}>
              <div style={{ position: "absolute", top: 8, right: 10 }}>
                <StatCardLink href="/payments" label="Review payment requests" />
              </div>
              <p style={{ fontSize: 11, color: "#6b7280", marginBottom: 2 }}>Pending Requests</p>
              {statsLoading ? <Skeleton className="h-6 w-20" /> : (
                <div style={{ fontSize: isValidator ? 20 : 18, fontWeight: 700, color: "#ca8a04" }}>
          ...
          <Card style={{ padding: 0 }}>
            <div style={{ padding: "10px 14px", position: "relative" }}>
              ...
              {statsLoading ? <Skeleton className="h-6 w-20" /> : (
                <div style={{ fontSize: isValidator ? 20 : 18, fontWeight: 700, color: "#16a34a" }}>
      ...
      <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(2, 1fr)" }} className="grid-projects">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between" style={{ padding: "16px 20px 8px" }}>
            ...
            <Link href="/projects" className="text-sm text-kaart-orange hover:underline">View all</Link>
          </CardHeader>
                  ...
                  >
                    <div>
                      <Link
                        ...
                        onClick={isValidator ? (e) => e.stopPropagation() : undefined}
                      >
                        {project.name}
                      </Link>
                      <p className="text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-2">
                      {isValidator && (project as Project & { unassigned?: boolean }).unassigned && (
        ...
        <Card>
          ...
          <CardContent className="space-y-3" style={{ padding: "8px 20px 16px" }}>
                ...
                {(requestsTotal ?? 0) > 0 ? (
                  <div className="rounded-lg bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 p-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="font-medium text-sm text-yellow-800 dark:text-yellow-200">Payment Request Pending</p>
                    </div>
                  ...
                  <div className="rounded-lg bg-green-50 dark:bg-green-950 p-3">
                    <p className="font-medium text-sm text-green-800 dark:text-green-200">
                      {formatCurrency(payable?.payable_total ?? vs?.payable_total).text} available!
                    </p>
                    <Button
                      variant="primary"
                      size="sm"
            ...
            <div className="flex flex-wrap gap-2">
              <Link href="/projects" className="inline-flex items-center rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium hover:bg-secondary/80 transition-colors" title="Browse all your assigned projects">
                View Projects
              </Link>
              {paymentsVisible && (
                <Link href="/payments" className="inline-flex items-center rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium hover:bg-secondary/80 transition-colors" title="View your payment history">
                  Payment History
                </Link>
              )}
              {isValidator ? (
                <Link href="/checklists" className="inline-flex items-center rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium hover:bg-secondary/80 transition-colors" title="View your checklists">
                  Checklists
                </Link>
              ) : (
                <Link href="/training" className="inline-flex items-center rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium hover:bg-secondary/80 transition-colors" title="View your training modules">
                  Training
                </Link>
              )}
              <Link href="/account" className="inline-flex items-center rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium hover:bg-secondary/80 transition-colors" title="Manage your account settings">
                Account
              </Link>
            </div>
          </CardContent>
            ...
            {statsLoading ? (
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <p className="text-xs text-muted-foreground mb-1">Contributions this month</p>
                  <div className="text-3xl font-bold">
                    <Val>{formatNumber(userStats?.total_contributions_for_month)}</Val>
                  </div>
                  <p className="text-xs mt-1">
                    {userStats?.month_contribution_change !== undefined && userStats.month_contribution_change >= 0 ? (
                      <span className="text-green-600">+{formatNumber(userStats.month_contribution_change).text} from last month</span>
                    ) : (
                      <span className="text-red-600">{formatNumber(userStats?.month_contribution_change).text} from last month</span>
                    )}
                  </p>
                </div>
                {userStats?.weekly_contributions_array && userStats.weekly_contributions_array.length > 0 && (
      ...
      {isValidator && (
        <>
          <ValidatorProjectsTable projects={projects} paymentsVisible={paymentsVisible} />
          <p className="text-sm text-muted-foreground">Double-click a project row to open it in the Tasking Manager.</p>
        </>
      )}
    </div>
  );
